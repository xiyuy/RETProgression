...
def load_test_dataset(data_dir, annotations_file, resolution=1000):
    ...
    test_dataset = JoslinData(
        ...
        img_dir="cropped_dataset_07082025",
...
def evaluate_model(model, test_loader, device, normalization_transform=None):
    ...
    return all_targets, all_predictions, all_probabilities, all_logits
...
def main():
    ...
    parser.add_argument('--annotations_file', type=str, default='clean_gradable_dr_test.csv',
    ...
    try:
        ...
        y_true, y_pred, y_prob, logits = evaluate_model(model, test_loader, device, normalization_transform)

        # ==== Save per-sample predictions (CSV) ====
        # Try to get stable IDs from the dataset (filenames). Fallback to row indices.
        if hasattr(test_dataset, "img_paths"):
            paths = test_dataset.img_paths
        elif hasattr(test_dataset, "image_paths"):
            paths = test_dataset.image_paths
        elif hasattr(test_dataset, "paths"):
            paths = test_dataset.paths
        else:
            paths = [f"idx_{i}" for i in range(len(y_true))]
        ...
        preds_file = os.path.join(args.output_dir, "predictions.csv")
        df.to_csv(preds_file, index=False)
        logging.info(f"Per-sample predictions saved to {preds_file}")
        # ===========================================
